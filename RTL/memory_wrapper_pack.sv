module memory_wrapper #(
    ...
    // ------------  Calculate ECC width   ---------------
    // ---------------------------------------------------
    localparam int ECC_BITS = calc_ecc_bits(DATA_WIDTH*WORDS_BLOCK);
    localparam int TOTAL_WIDTH = DATA_WIDTH*WORDS_BLOCK + ECC_BITS;
    ...
    ecc_decoder #(
        ...
        .ecc_in(sram_dout[DATA_WIDTH*WORDS_BLOCK-1+ECC_BITS:DATA_WIDTH*WORDS_BLOCK]),
        //.ecc_length(ECC_BITS),
        .d_out(q_out),
	...
	// ---------------------------------------------------
    // ------     Function to calculate ECC bits      ----
    // ---------------------------------------------------
	
	// k = number of bits (information)
	// find the smallest r, satarting with r = 1
    function automatic int calc_ecc_bits (int k);
        int r = 1;
        while ((2**r) < (k + r + 1)) r++; // compute the smallest r
        return r + 1; //add 1 for overall parity (2 errors detection)
    endfunction

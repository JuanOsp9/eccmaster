...
module tb_bch_ecc;
    ...
    // -------------------------------------------------------------------------
    // 1. Parameters (Must match your Design)
    // -------------------------------------------------------------------------
    parameter DATA_WIDTH  = 16;
    parameter CHECK_WIDTH = 10;
    parameter CODE_WIDTH  = 26; // 16 + 10

    // -------------------------------------------------------------------------
    // 2. Signals
    // -------------------------------------------------------------------------
    logic [DATA_WIDTH-1:0]  data_i;
    logic [CODE_WIDTH-1:0]  force_error_vector; // The "Error Injection" port
    ...
    initial begin
        ...
        // Initialization
        data_i = 16'hAAAA; // Random data pattern (1010...)
        force_error_vector = '0;
        #10;

        // =====================================================================
        // TEST CASE 1: No Errors (Transparency Check)
        // =====================================================================
        ...
        for (int i = 0; i < CODE_WIDTH; i++) begin
            ...
            // Check: Data should be corrected, Error Flag should be HIGH
        ...
        for (int i = 0; i < CODE_WIDTH; i++) begin
            for (int j = i + 1; j < CODE_WIDTH; j++) begin
                ...
                // Check: Data should be corrected (t=2 capability)
        ...
        // =====================================================================
        // TEST CASE 4: Triple Bit Error (Expect Failure)
        // =====================================================================
        $display("\nTest 4: Triple Bit Error Check (Should FAIL to correct)");
        force_error_vector = '0;
        ...
        if (data_o !== data_i)
            ...
            $display("  [INFO] Lucky triple error was corrected (rare but possible).");

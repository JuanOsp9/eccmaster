module memory_wrapper_fv #(
    ...
    parameter int CODE_WIDTH	 	= 68,  // DATA*2 + CHECK_H + CHECK_V
...
logic [ADDR_WIDTH-1:0] fv_addr_base;
    ...
    property p_word_packing_error;
        ...
        ##1 ( !pi_cen && pi_wen && pi_addr == fv_addr_base + WORDS_BLOCK - 1 && fv_ecc_enc_data == memory_wrapper_16bit_B2.sram_dout[DATA_WIDTH*WORDS_BLOCK-1:0])[*2]
		|->
		po_q == fv_data_in
        //&& po_err_corr
	;endproperty
    ...
    && !pi_wen
    |->
    {fv_ecc_enc_data, memory_wrapper_16bit_B2.encoder_inst.V_out, memory_wrapper_16bit_B2.encoder_inst.H_out} == {fv_data_in_pack,check_bits}
    ;endproperty
...
// 1. Declare the mask
    ...
    localparam int RANGE_START = CODE_WIDTH - 20; // The lower index
    localparam int RANGE_END   = CODE_WIDTH - 16;  // The higher index
    ...
    localparam int RANGE_END   = CODE_WIDTH - 20;  // Higher index
    localparam int RANGE_START = CODE_WIDTH - 16; // Lower index
...
logic [CHECK_H_WIDTH+CHECK_V_WIDTH-1:0] check_bits;
    ...
    assign s0 = fv_data_in_pack[3:0];   assign s1 = fv_data_in_pack[7:4];
    assign s2 = fv_data_in_pack[11:8];  assign s3 = fv_data_in_pack[15:12];
    assign s4 = fv_data_in_pack[19:16]; assign s5 = fv_data_in_pack[23:20];
    assign s6 = fv_data_in_pack[27:24]; assign s7 = fv_data_in_pack[31:28];
    ...
    assign V_out[3:0]   = s0 ^ s4;
    assign V_out[7:4]   = s1 ^ s5;
    assign V_out[11:8]  = s2 ^ s6;
    assign V_out[15:12] = s3 ^ s7;

    // Encoder-Reuse Technique (ERT) Multiplexer
    assign H_out[4:0]   = H_gen[4:0];
    assign H_out[9:5]   = H_gen[9:5];
    assign H_out[14:10] = H_gen[14:10];
    assign H_out[19:15] = H_gen[19:15];
    ...
    bind memory_wrapper_16bit_B2 memory_wrapper_fv #(

...
module tb_bch_ecc;
    ...
    // -------------------------------------------------------------------------
    // 1. Parameters (UPDATED FOR 32-BIT)
    // -------------------------------------------------------------------------
    parameter DATA_WIDTH  = 32;      // Changed from 16
    parameter CHECK_WIDTH = 12;      // Changed from 10 (Required for 32-bit DEC)
    parameter CODE_WIDTH  = 44;      // 32 + 12 = 44
    ...
    initial begin
        ...
        // Initialization (UPDATED PATTERN)
        data_i = 32'hAAAA_AAAA; // Expanded to fill 32 bits
        ...
        // NOTE: This loop is now O(N^2) for N=44. It will run ~946 iterations.
        for (int i = 0; i < CODE_WIDTH; i++) begin
